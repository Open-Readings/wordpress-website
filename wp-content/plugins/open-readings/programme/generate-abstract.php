...
{
    $title = $abstract_fields[$hash_id]['title'];
    ...
    $replacements = array(
        ...
        '${author_list}' => $author_list,
        '${session}' => $abstract_fields[$hash_id]['session'],
        '${research_area}' => $abstract_fields[$hash_id]['research_area']
...
function get_abstract_data()
{
    ...
    $results = $wpdb->get_results("SELECT person_hash_id, title, authors, affiliations, `references`, content, images, session_id FROM wp_or_registration_presentations");



    $abstract_fields = array();
    foreach ($results as $result) {
        $abstract_fields[$result->person_hash_id]['title'] = $result->title;
        ...
        $research_area = $wpdb->get_results("SELECT research_area FROM wp_or_registration WHERE hash_id = '$result->person_hash_id'");
        $abstract_fields[$result->person_hash_id]['research_area'] = $research_area[0]->research_area;
